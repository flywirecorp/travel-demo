...
export default {
  ...
  mounted: function() {
    const handlers = {
      onClick: this.onPay,
      onInvalidInput: this.onPayErrorHandler
    };

    window.FlywirePayment.button(handlers).render("#buttonContainer");
  },
  methods: {
    ...
    onPayErrorHandler: function(errs) {
      this.$store.dispatch('saveCheckoutErrors', errs)
          .then(() => {
              this.$router.push('/billing-info');
          });
    }
